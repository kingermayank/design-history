/**
 * design-history in-app overlay (injected by the Vite plugin during dev).
 *
...
class Overlay {
  ...
  private effectiveFrameSrc(s: Snapshot): string | null {
    ...
    return `${BASE}/snapshots/${encodeURIComponent(eff.id)}/${encodeURIComponent(frame.file)}`;
  }
